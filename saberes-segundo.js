/* TeleVerso Educativo · Saberes y pensamiento científico · 2.º de Telesecundaria · PPA 1
   ...
   Ruta independiente de Saberes 3.º.
   Mecánicas: clasificación, sopa de letras, cálculo, emparejamiento, laboratorio de fuentes,
   detective de gráficas, crucigrama real, relaciones, ruta de interpretación y escape integrador.
*/
(() => {
  ...
  const meta=(m,desc,paper)=>Object.assign(m,{field:FIELD,fieldTitle:FIELD_TITLE,level:'2.º de Telesecundaria',grade:2,cognitiveLevel:'comprender-aplicar-analizar',desc,paper});
  ...
  Object.assign(missionCatalog,{
    sp2_1:meta({id:'sp2_1',title:'R1 · Detective de datos',type:'sp2-classify',icon:'🔎',
      ...
      ]},'Clasifica ejemplos para distinguir población, muestra y tipos de datos antes de construir tablas o gráficas.',`${PAPER} p. 5`),
    ...
    sp2_2:meta({id:'sp2_2',title:'R2 · Sopa estadística',type:'sp2-wordsearch',icon:'🔤',
      ...
      ]},'Encuentra conceptos estadísticos en una sopa interactiva y relaciona cada palabra con su definición.',`${PAPER} p. 8`),
    ...
    sp2_3:meta({id:'sp2_3',title:'R3 · Laboratorio de cálculos',type:'sp2-calc',icon:'🧮',
      ...
      ]},'Resuelve cálculos de media, mediana, moda y rango; después interpreta qué aporta cada medida.',`${PAPER} p. 10`),
    ...
    sp2_4:meta({id:'sp2_4',title:'R4 · Selector de gráficas',type:'sp2-match',icon:'📊',
      ...
      ]},'Empareja cada propósito con la representación que permite comunicar mejor el comportamiento de los datos.',`${PAPER} p. 12`),
    ...
    sp2_5:meta({id:'sp2_5',title:'R5 · Laboratorio de fuentes',type:'sp2-source',icon:'🧪',
      ...
      ]},'Evalúa la trazabilidad de una fuente antes de incorporar datos a un artículo informativo.',`${PAPER} p. 18`),
    ...
    sp2_6:meta({id:'sp2_6',title:'R6 · Detective de gráficas engañosas',type:'sp2-choice',icon:'🕵️',
      ...
      ]},'Analiza escalas, ejes, periodos y fuentes para detectar representaciones que pueden distorsionar un mensaje.',`${PAPER} p. 20`),
    ...
    sp2_7:meta({id:'sp2_7',title:'R7 · Crucigrama de estadística y equidad',type:'sp2-crossword',icon:'✍️',size:13,
      ...
      ]},'Completa un crucigrama real con conceptos que usarás para organizar datos y explicar límites del estudio.',`${PAPER} p. 27`),
    ...
    sp2_8:meta({id:'sp2_8',title:'R8 · Evidencia y representación',type:'sp2-match',icon:'🔗',
      ...
      ]},'Relaciona distintos tipos de evidencia con formas de representación útiles para el proyecto sobre equidad.',`${PAPER} p. 28`),
    ...
    sp2_9:meta({id:'sp2_9',title:'R9 · Ruta de interpretación',type:'sp2-story',icon:'🧭',
      ...
      ]},'Toma decisiones de interpretación para evitar generalizaciones y explicar con honestidad lo que sí muestran los datos.',`${PAPER} p. 30`),
    ...
    sp2_10:meta({id:'sp2_10',title:'R10 · Escape DATOS',type:'sp2-escape',icon:'🔐',
      ...
      ]},'Abre cinco candados integrando recolección, cálculo, representación, fuentes e interpretación. Código final: DATOS.',`${PAPER} p. 31`)
  });
  ...
  function ensureUI(){
    ...
    if(selector && !document.getElementById('tv-btn-spc2')){
      ...
      if(grid){
        grid.classList.remove('lg:grid-cols-4'); grid.classList.add('lg:grid-cols-5');
        ...
        b.innerHTML='<span class="block text-[10px] font-black uppercase tracking-wider text-emerald-300">Campo formativo · 2.º grado</span><strong class="block text-white mt-1 text-sm">Saberes · PPA 1</strong><span class="block text-[10px] text-slate-400 mt-1">Conocimiento científico y datos</span>';
        grid.appendChild(b);
      }
    }
    ...
    if(anchor && !document.getElementById('tv-badges-spc2')){
      ...
      d.innerHTML='<h4 class="text-sm font-black text-slate-300 uppercase tracking-wider mb-3 flex items-center gap-2"><i class="fa-solid fa-flask text-emerald-400"></i> Insignias Cooperativas · Saberes 2.º</h4><div class="grid grid-cols-1 sm:grid-cols-3 gap-4"><div id="sp2-badge-data" class="glass-card p-4 rounded-2xl opacity-50"><strong class="text-white">Explorador de datos</strong><span class="block text-[11px] text-slate-400">R1-R3 · muestra, conceptos y cálculos.</span></div><div id="sp2-badge-graph" class="glass-card p-4 rounded-2xl opacity-50"><strong class="text-white">Analista gráfico</strong><span class="block text-[11px] text-slate-400">R4-R6 · gráficas, fuentes y lectura crítica.</span></div><div id="sp2-badge-science" class="glass-card p-4 rounded-2xl opacity-50"><strong class="text-white">Comunicador científico</strong><span class="block text-[11px] text-slate-400">R7-R10 · equidad, interpretación y síntesis.</span></div></div>';
      anchor.after(d);
    }
    syncVisibility();
  }
  ...
  function renderGrid(){
    ...
    const ms=missions();const heading=c.previousElementSibling;if(heading&&heading.tagName==='H4')heading.innerHTML='<i class="fa-solid fa-flask text-emerald-400"></i> Saberes y pensamiento científico · 2.º grado · PPA 1 · 10 retos';
    c.innerHTML=ms.map((m,i)=>`<div class="glass-card p-5 rounded-3xl flex flex-col justify-between border-t-2 border-t-emerald-500"><div><div class="flex justify-between items-center mb-3"><span class="px-2.5 py-0.5 bg-emerald-500/20 text-emerald-300 text-[10px] font-black uppercase rounded-lg border border-emerald-500/30">Misión ${i+1}</span><i id="badge-mission-${m.id}" class="fa-solid fa-lock text-slate-500 text-base"></i></div><h4 class="text-base font-bold text-white mb-1.5">${m.icon} ${m.title}</h4><p class="text-slate-300 text-xs mb-2 leading-relaxed">${m.desc}</p><p class="text-amber-300/90 text-[11px] font-semibold mb-4">Actividad equivalente: ${m.paper}</p></div><button onclick="launchMission('${m.id}')" class="w-full py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl text-xs transition"><i class="fa-solid fa-play mr-1"></i> Iniciar reto</button></div>`).join('');
  ...
  const head=(m,p='')=>`<div class="flex flex-wrap justify-between gap-2 text-xs font-bold text-emerald-300"><span>${p}</span><span class="text-amber-300">${m.paper}</span></div><h3 class="text-xl font-black text-white">${m.icon} ${m.title}</h3><p class="text-slate-300 text-xs">${m.desc}</p>`;
  ...
  function choiceRender(){const m=missionCatalog[activeMissionKey],s=missionGameState,a=document.getElementById('mission-content-area');if(s.idx>=m.qs.length){saveMissionResults('Reto completado. Escribe en el cuaderno dos señales de alerta que revisarás en una gráfica publicada.');return;}const[q,opts]=m.qs[s.idx];a.innerHTML=`<div class="space-y-4 animate-pop">${head(m,`Pregunta ${s.idx+1}/${m.qs.length} · Aciertos ${s.hits}`)}<div class="glass-card p-5 rounded-2xl"><p class="text-base sm:text-lg font-black text-center mb-4">${q}</p><div class="grid gap-3">${opts.map((o,i)=>`<button onclick="sp2ChoiceAnswer(${i})" class="p-4 bg-slate-800 hover:bg-emerald-600 rounded-xl text-left text-sm font-bold">${String.fromCharCode(65+i)} · ${o}</button>`).join('')}</div></div></div>`;}
  ...
  function calcRender(){const m=missionCatalog[activeMissionKey],s=missionGameState,a=document.getElementById('mission-content-area');if(s.idx>=m.problems.length){saveMissionResults('Laboratorio completado. Compara tus procedimientos con los de la página 10 del cuaderno.');return;}const p=m.problems[s.idx];a.innerHTML=`<div class="space-y-4 animate-pop">${head(m,`Cálculo ${s.idx+1}/${m.problems.length} · Aciertos ${s.hits}`)}<div class="glass-card p-6 rounded-2xl text-center"><div class="text-4xl mb-3">🧮</div><p class="text-lg font-black mb-4">${p.q}</p><input id="sp2-calc-answer" inputmode="decimal" class="w-full max-w-xs bg-slate-950 border border-emerald-500/40 rounded-xl px-4 py-3 text-center text-xl font-black text-white" placeholder="Escribe el resultado"><button onclick="sp2CalcCheck()" class="mt-4 w-full max-w-xs py-3 bg-emerald-600 hover:bg-emerald-500 rounded-xl font-black">Comprobar</button></div></div>`;}
  ...
  function matchRender(){const m=missionCatalog[activeMissionKey],s=missionGameState,a=document.getElementById('mission-content-area');if(s.done.length===m.pairs.length){saveMissionResults('Emparejamiento completado. Justifica en el cuaderno una de tus elecciones.');return;}a.innerHTML=`<div class="space-y-4 animate-pop">${head(m,`Parejas ${s.done.length}/${m.pairs.length}`)}<p class="text-xs text-slate-400">Elige una tarjeta morada y después su pareja verde.</p><div class="grid grid-cols-1 md:grid-cols-2 gap-4"><div class="space-y-2">${s.left.map(o=>`<button ${s.done.includes(o.i)?'disabled':''} onclick="sp2MatchLeft(${o.i})" class="w-full p-3 rounded-xl text-left font-bold text-xs ${s.done.includes(o.i)?'opacity-30 bg-slate-800':s.sel===o.i?'bg-violet-500':'bg-violet-800 hover:bg-violet-700'}">${o.t}</button>`).join('')}</div><div class="space-y-2">${s.right.map(o=>`<button ${s.done.includes(o.i)?'disabled':''} onclick="sp2MatchRight(${o.i})" class="w-full p-3 rounded-xl text-left font-semibold text-xs ${s.done.includes(o.i)?'opacity-30 bg-slate-800':'bg-emerald-800 hover:bg-emerald-700'}">${o.t}</button>`).join('')}</div></div></div>`;}
  ...
  function escapeRender(){const m=missionCatalog[activeMissionKey],s=missionGameState,a=document.getElementById('mission-content-area');if(s.idx>=m.locks.length){saveMissionResults(`Escape completado. Código: ${s.letters.join('')}. Explica por qué DATOS no significa sólo “números”.`);return;}const lk=m.locks[s.idx];a.innerHTML=`<div class="space-y-4 animate-pop">${head(m,`Candado ${s.idx+1}/5 · Código: ${s.letters.join(' ')} ${'_ '.repeat(5-s.letters.length)}`)}<div class="glass-card p-6 rounded-2xl border border-emerald-500/30"><div class="text-center text-4xl mb-3">🔒</div><h4 class="text-center text-lg font-black text-emerald-300 mb-3">${lk.title}</h4><p class="font-semibold mb-4">${lk.q}</p><div class="grid gap-3">${lk.opts.map((o,i)=>`<button onclick="sp2EscapeAnswer(${i})" class="p-3.5 bg-slate-800 hover:bg-emerald-600 rounded-xl text-left font-bold text-sm">${o}</button>`).join('')}</div></div></div>`;}
