/* TeleVerso Educativo · Acceso por grado escolar
   ...
   - 2.º: Lenguajes 2.º PPA 1 (lenguajes2)
   - 3.º: Ética, Lenguajes 3.º y Saberes
*/
(() => {
  if (typeof missionCatalog === 'undefined') return;

  const FIELD_GRADE = { lenguajes2: 2, ens: 3, lenguajes: 3, saberes: 3 };
  const GRADE_FIELDS = { 2: ['lenguajes2'], 3: ['ens', 'lenguajes', 'saberes'] };
  ...
  function applyStudentGradeUI() {
    ...
    const map = {'tv-btn-ens':3,'tv-btn-leng':3,'tv-btn-spc':3,'tv-btn-leng2':2};
    ...
    if (grid) {
      ...
      if (grade === 3) grid.classList.add('sm:grid-cols-3'); else grid.classList.add('sm:grid-cols-1');
    }
